...
class StellarsisClient:
    def __init__(self):
        # 配置服务器地址
        self.base_url = "http://192.168.121.62:5000"  # 可以修改为实际的服务器地址
    ...
    def create_main_layout(self):
        ...
        # 用户信息和导航按钮
        ttk.Label(top_frame, text=f"用户: {self.current_user}", font=('', 10, 'bold')).pack(side=LEFT, padx=10, pady=5)
        
        # 在线用户数显示
        self.online_count_label = ttk.Label(top_frame, text="在线: 0", font=('', 10))
    ...
    def show_chat_interface(self):
        ...
        # 创建聊天导航
        ttk.Label(self.nav_frame, text="聊天室", font=('', 12, 'bold')).pack(pady=10)
    ...
    def load_chat_rooms(self):
        ...
        try:
            ...
            for div in room_cards:
                room = {}
                room['name'] = div.find('h3').get_text()
                room['id'] = int(div.find('span', class_="unread-badge")["data-room-id"])
                room['description'] = div.find('p', class_="room-description").get_text()
                rooms.append(room)
            ...
            for room in rooms:
                btn = ttk.Button(
                    self.nav_frame,
                    text=room["name"],
                    command=lambda r=room: self.select_chat_room(r["id"], r["name"], r["description"]),
                    bootstyle="outline"
                )
    ...
    def select_chat_room(self, room_id, room_name, room_description):
        """选择聊天室"""
        ...
        # 创建聊天界面
        self.create_chat_room_interface(room_name, room_description)
        
        # 加载聊天历史
        self.load_chat_history()
    
    def create_chat_room_interface(self, room_name, room_description):
        """创建聊天室界面"""
        # 房间标题
        ttk.Label(self.content_frame, text=f"房间: {room_name}", font=('', 14, 'bold')).pack(pady=5)

        ttk.Label(self.content_frame, text=room_description, font=('', 14, 'bold')).pack(pady=5)
    ...
    def show_forum_interface(self):
        ...
        # 创建论坛导航
        ttk.Label(self.nav_frame, text="论坛分区", font=('', 12, 'bold')).pack(pady=10)
    ...
    def load_forum_sections(self):
        """加载论坛分区"""
        try:
            response = self.session.get(f"{self.base_url}/forum")
            if response.status_code == 200:
                soup = BeautifulSoup(response.text, 'lxml')
                section_divs = soup.find_all('div', class_='section-card')
                sections = []
                for div in section_divs:
                    section = {}
                    section['name'] = div.find('h2', class_="section-title").get_text()
                    section['id'] = int(div.find('span', class_="unread-badge")["data-section-id"])
                    section['description'] = div.find('p', class_="section-description").get_text()
            ...
            for section in sections:
                btn = ttk.Button(
                    self.nav_frame,
                    text=section["name"],
                    command=lambda s=section: self.select_forum_section(s["id"], s["name"], s["description"]),
                    bootstyle="outline"
                )
                btn.pack(fill=X, padx=5, pady=2)
    ...
    def select_forum_section(self, section_id, section_name, section_description):
        """选择论坛分区"""
        ...
        # 创建论坛分区界面
        self.create_forum_section_interface(section_name, section_description)
        
        # 加载分区主题
        self.load_forum_threads()
    
    def create_forum_section_interface(self, section_name, section_description):
        """创建论坛分区界面"""
        # 分区标题
        ttk.Label(self.content_frame, text=f"分区: {section_name}", font=('', 14, 'bold')).pack(pady=5)
        ttk.Label(self.content_frame, text=section_description, font=('', 10)).pack(pady=5)
        ...
        # 绑定双击事件
        self.threads_listbox.bind('<Double-1>', lambda e: self.view_forum_thread(e))
    ...
    def load_forum_threads(self):
        ...
        try:
            # 尝试通过API获取分区主题列表
            response = self.session.get(f"{self.base_url}/forum/section/{self.current_section_id}")
            if response.status_code == 200:
                soup = BeautifulSoup(response.text, 'lxml')
                thread_divs = soup.find_all('li', class_='thread-item')
                threads = []
                for div in thread_divs:
                    thread = {}
                    thread['title'] = div.find('a').get_text()
                    thread['id'] = int(div.find('a')["href"].split('/')[-1])
                    thread['author'] = div.find('span').get_text().replace('作者:', '').strip()
                    thread['replies'] = int(div.find_all('span')[2].get_text().split(':')[-1].strip())
            ...
            for thread in threads:
                display_text = f"{thread['title']} (作者: {thread['author']}, 回复: {thread['replies']}, ID: {thread['id']})"
    ...
    def view_forum_thread(self, e):
        ...
        thread_id = self.thread_info_list[thread_index]['id']
        ...
        # 创建主题查看界面
        self.create_forum_thread_interface(thread_id)
    
    def create_forum_thread_interface(self, thread_id):
        """创建论坛主题界面"""
        # 返回按钮
        ttk.Button(
            self.content_frame, 
        ...
        # 模拟加载主题内容
        response = self.session.get(f"{self.base_url}/forum/thread/{thread_id}")
        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'lxml')
            content_div = soup.find('div', class_='thread-container')
            print(content_div)
            sample_content = content_div.find('script', class_="raw-content").get_text().encode() if content_div else "无法加载主题内容。"
    ...
    def show_profile(self):
        ...
        # 用户名
        ttk.Label(profile_window, text=f"用户名: {self.current_user}", font=('', 12, 'bold')).pack(pady=10)
        
        # 密码修改
        ttk.Label(profile_window, text="修改密码", font=('', 10, 'bold')).pack(pady=(20, 5))
